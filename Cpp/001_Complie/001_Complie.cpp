// 001_Compile.cpp : 이 파일에는 'main' 함수가 포함됩니다. 거기서 프로그램 실행이 시작되고 종료됩니다.
//

// 완성된 코드
// 코드란 C++을 문법을 지켜서 어떠한 프로그램을 만드는 언어
// 아래의 내용이 뭔지는 모르겠지만 어쨌든 코드라고 부르고
// 아래의 내용이 문법이 정확하다면 F5를 눌렀을시
// 컴파일 과정을 통해서 exe파일이 만들어 진다.

// 컴파일은 다음의 4가지 단계를 가지게 되는데.
// 전처리기 컴파일러 어셈블러 링커
// 다 설명할수는 없고
// 전처리기만 설명할겁니다.
...
// 전처리기를 이해하려면
// 전처리문을 이해해야하는데.
// 전처리 문은 컴파일이기 되기전에 컴파일 이전에 처리된다고 해서 전처리라고 
// 이런 전처리구문들을 전처리문이라고 부른다.
// 1. 실제적인 코드와는 관련이 없는 전처리문
// 2. 코드로 치환되어야 하는 전처리문

// 1. 실제적인 코드와는 관련이 없거나.
// <= 주석 
// 이건 xxxx를 하는 xxx라는 걸 설명하고 싶고. 
// 설명 혹은 첨삭 혹은 메모
// 전처리기는 이게 실제 exe에 들어갈 내용이 아니라는 걸 알고 있기 때문에.
// 다 지워버립니다.

// 2. 코드로 치환되어야 하는 전처리문
// 보통 코드로 치환되거나 의미를 가지는 전처리문에는
// #이 붙습니다. 
// #include <iostream> // 이녀석도 전처리문인데
