...
public:
	...
	//
	// 4. 소멸자에는 인자를 넣어 줄 수 없다.
	// 
	// 5. 어떠한 소멸자도 만들지 않으면 기본 소멸자가 생성된다.
	//
	// 6. 부모형태로 자식클래스를 관리할 경우 소멸자가 호출될때
	//    부모형의 소멸자만 호출되는 경우가 있는데
	//    부모의 소멸자에 virtual 자식의 소멸자에 override
	//    를 적어 주자.
	//
	~Player()
	{
